...
if (isset($_POST['generateSpecificReport'])) {
	...
	if (mysqli_num_rows($result) > 0) {
        while ($row = $result->fetch_object()) {
			...
			$query = "SELECT COUNT(*) AS deduction_id
					FROM tbl_deductions
					WHERE emp_id = $user
					AND DATE_FORMAT(created_at, '%m/%d/%Y') BETWEEN '$start' AND '$end'";

			$get_total_timeouts = mysqli_query($conn, $query);

			if ($get_total_timeouts) {

				$rowData = mysqli_fetch_assoc($get_total_timeouts);
				if ($rowData) {
					$column1Value = $rowData['deduction_id'];
	 				addHeader($pdf);
	    ...
	    $pdf->Cell(95, 10, 'FIRSTNAME: NO DATA ', 1, 0);
	    $pdf->Cell(50, 10, 'DATE OF BIRTH: NO DATA ', 1, 0);
	    $pdf->Cell(45, 10, 'POSITION: NO DATA ', 1, 1);
	    
	    $pdf->Cell(47, 10, 'START DATE: NO DATA ', 1, 0);
	    $pdf->Cell(48, 10, 'END DATE: NO DATA ', 1, 0);
	    $pdf->Cell(50, 10, 'TOTAL HOURS: NO DATA ', 1, 0);
	    $pdf->Cell(45, 10, 'PAY PERIOD: NO DATA ', 1, 1);
	    ...
	    $pdf->MultiCell(95, 10, 'BASIC SALARY: NO DATA '."\n".'ALLOWANCE: _______________', 'LRT', false);
	    $pdf->MultiCell(95, 10, '', 'L', false);

	    $nextX = $pdf->GetX() + 95;
	    $nextY = $pdf->GetY() - 30;
	    $pdf->SetXY($nextX, $nextY);
	    $pdf->MultiCell(95, 10, 'NO. OF MISSED TIMEOUTS: NO DATA '."\n".'SSS: _______________'."\n".'PHILHEALTH: _______________', 'LTRB', false);
    ...
    if (mysqli_num_rows($result) > 0) {
        while ($row = $result->fetch_object()) {
            ...
            if ($get_total_timeouts) {
                $rowData = mysqli_fetch_assoc($get_total_timeouts);
                if ($rowData) {
                    $missed_timeouts = $rowData['deduction_id'];
                    ...
                    $nextY = $pdf->GetY() - 30;
                    $pdf->SetXY($nextX, $nextY);
                    $pdf->MultiCell(95, 10, 'NO. OF MISSED TIMEOUTS: ' . $missed_timeouts . "\n" . 'SSS: _______________' . "\n" . 'PHILHEALTH: _______________', 'LTRB', false);
